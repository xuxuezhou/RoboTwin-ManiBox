...
def get_model(usr_args): # keep 
    model_name = usr_args['setting']
    checkpoint_id = usr_args['checkpoint_id']
    left_arm_dim, right_arm_dim, rdt_step = usr_args['left_arm_dim'], usr_args['right_arm_dim'], usr_args['rdt_step']
    try:
        rdt = RDT(os.path.join(parent_directory, f"checkpoints/{model_name}/checkpoint-{checkpoint_id}/pytorch_model/mp_rank_00_model_states.pt"), usr_args["task_name"], left_arm_dim, right_arm_dim, rdt_step)
    except:
        rdt = RDT(os.path.join(parent_directory, f"checkpoints/{model_name}/checkpoint-{checkpoint_id}/"), usr_args["task_name"], left_arm_dim, right_arm_dim, rdt_step)
    return rdt
...
def eval(TASK_ENV, model, observation):
    ...
    if model.observation_window is None: # Force an update of the observation at the first frame to avoid an empty observation window
