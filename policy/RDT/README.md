...
First, create the `data` and `training_data` folders in the `policy/RDT` directory:
```bash
mkdir data && mkdir training_data
```
...
# After running, the data will be saved to `policy/RDT/data` by default.
bash process_data_rdt.sh $task_name $head_camera_type $expert_data_num $gpu_id
```

If success, you will find the `${task_name}_${expert_data_num}` folder under `policy/RDT/data`, with the following data structure:
```
data/
...
### 4.1 Prepare Data
Copy all the data you wish to use for training from `data` into `training_data/${model_name}`. If you have multiple tasks with different data, simply copy them in the same way.
