...
def main(usr_args):
    ...
    save_dir_folder_name = str(args['task_name']) + '/' + f'{policy_name}' + '/' + f'{embodiment_name}' + '-m' + str(int(args['augmentation']['messy_table'])) + '_b' + str(int(args['augmentation']['random_background'])) \
    ...
    save_dir = Path(f'eval_result/{save_dir_folder_name}/{setting}/{current_time}')
    ...
    st_seed, suc_num = eval_policy(task_name, TASK_ENV, args, model, st_seed, test_num=test_num, video_size=video_size, instruction_type=instruction_type)
...
def eval_policy(task_name, TASK_ENV, args, model, st_seed, test_num=100, video_size=None, instruction_type=None):
    ...
    while succ_seed < test_num:
        ...
        # TASK_ENV.apply_policy_demo(policy)
        succ = False
        reset_func(model)
        ...
        print(f"[ {task_name} | {args['policy_name']} | {args['setting']} ]\nSuccess rate:{TASK_ENV.suc}/{TASK_ENV.test_num} => {round(TASK_ENV.suc/TASK_ENV.test_num*100, 1)}%, current seed: {now_seed}\n")
