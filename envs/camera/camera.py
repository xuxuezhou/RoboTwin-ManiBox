...
class Camera():
    def __init__(self, bias=0, random_head_camera_dis=0, **kwags):
        ...
        self.head_camera_type = kwags.get('head_camera_type', 'D435')
        self.wrist_camera_type = kwags.get('wrist_camera_type', 'D435')

        self.collect_head_camera = kwags.get('collect_head_camera', True)
        self.collect_wrist_camera = kwags.get('collect_wrist_camera', True)
    ...
    def load_camera(self, scene):
        ...
        for i,camera_info in enumerate(self.static_camera_info_list):
            ...
            if camera_info.get('left') == None:
                camera_info['left'] = [-camera_info['forward'][1], camera_info['forward'][0]] + [0]
            ...
            if camera_info['name'] == 'head_camera':
                if self.collect_head_camera:
                    ...
                    camera, camera_config = create_camera(camera_info, random_head_camera_dis=self.random_head_camera_dis)
    ...
    def get_pcd(self, is_conbine = False):
        ...
        if is_conbine:
            ...
            if self.collect_wrist_camera:
                conbine_pcd = np.vstack((_get_camera_pcd(self.left_camera), _get_camera_pcd(self.right_camera)))
            for camera,camera_name in zip(self.static_camera_list, self.static_camera_name):
                if camera_name == 'head_camera':
                    if self.collect_head_camera:
