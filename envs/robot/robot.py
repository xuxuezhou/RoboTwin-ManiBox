...
class Robot():
    ...
    def create_target_pose_list(self, origin_pose, center_pose, arm_tag = None):
        ...
        for i in range(CONFIGS.ROTATE_NUM):            
            now_pose = transforms.rotate_along_axis(origin_pose, center_pose, [0, 1, 0], rotate_step * i + rotate_lim[0], axis_type='target', towards=[0, -1, 0])
    ...
    def init_joints(self):
        ...
        def get_gripper_joints(find, gripper_name:str):
            gripper = [(find(gripper_name['base']), 1., 0.)]
    ...
    def right_plan_path(self, target_pose, constraint_pose = None, use_point_cloud=False, use_attach=False, last_qpos = None):
        ...
        if last_qpos is None:
            now_qpos = self.right_entity.get_qpos()
            # now_qpos = self.get_right_arm_jointState()[:-1]
        else:
            now_qpos = deepcopy(last_qpos)
    ...
    def get_normal_real_gripper_val(self):
        ...
        normal_right_gripper_val = (self.right_gripper[0][0].get_drive_target()[0] - self.right_gripper_scale[0]) / (self.left_gripper_scale[1] - self.left_gripper_scale[0])
    ...
    def set_gripper(self, gripper_val, arm_tag, gripper_eps = 0.1): # gripper_val in [0,1]
        ...
        for joint in joints:
            ...
            drive_velocity_target = np.clip(drive_target - real_joint.drive_target, -1., 1.) * 0.05
