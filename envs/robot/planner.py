...
try:
    ...
    class CuroboPlanner():
        def __init__(self, robot_origion_pose, active_joints_name, entity, yml_path=None, scene=None):
            super().__init__()
            ta.setup_logging("CRITICAL") # hide logging
                ...
                world_config = {
                    "cuboid": {
                        "table": {
                            ...
                            "pose": [self.robot_origion_pose.p[1], 0.0, 0.74 - self.robot_origion_pose.p[2], 1, 0, 0, 0.0],  # x, y, z, qw, qx, qy, qz
                        },
                    }
                }
        ...
        def plan_batch(self, curr_joint_pos, target_gripper_pose_list, constraint_pose=None, arms_tag=None):
            ...
            for target_gripper_pose in target_gripper_pose_list:
                world_target_pose = np.concatenate([np.array(target_gripper_pose.p), \
                                                np.array(target_gripper_pose.q)])
                base_target_pose_p, base_target_pose_q = \
                self._trans_from_world_to_base(world_base_pose, world_target_pose)
